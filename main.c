...
/*
// generate new player
void generatePlayer(int n, int initEnergy)
{
	int i;
	for(i=0;i<n;i++)
	{
...
int main(int argc, const char * argv[]) {
    ...
    while (fscanf(fp, "%s %i %i %i", name, &type, &credit, &energy ) == 4 ) //read a node parameter set
    {
        ...
        //mObj_genNode(name, type, credit, energy); 
...
//printf("(%s)", smmObj_getTypeName(SMMNODE_TYPE_LECTURE));
    ...
    cur_player = (player_t*)malloc(player_nr*sizeof(player_t)); // allocate player_t size memory to cur_player
